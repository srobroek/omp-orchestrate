...
1. After claiming, read the bead's numbered `acceptance_criteria` first, then inspect cited source and relevant tests in one read wave. Verify citations against current code; report drift instead of redoing completed work.
...
4. Run every acceptance criterion's verification and report actual results. Never claim success over failed verification.
