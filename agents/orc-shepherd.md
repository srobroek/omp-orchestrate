...
4. Inspect branch/base mergeability with `orc_conflict_probe`, actual CI with its `mode="ci"`, and every configured exact-head review bot with `orc_bot_review_probe`. Confirmed branch/base conflict → CONFLICT remediation. Missing or unknown conflict evidence and declined/rate-limited checks → BLOCKED. Pending/stale → IDLE. Actionable findings enter the bounded bot-fix loop below. A closed gate is not success.
...
For an actionable bot round, collect the union of findings before creating one fix bead. Identify each issue by its GitHub review-thread node id; when no thread exists, use the bot review URL plus its finding fingerprint. Compare those identities and findings with prior bot-fix beads. `metadata.bot_issue_attempts[issue_key]` counts completed fix attempts, not observations: initialize a new issue at zero, and increment it only after the architect integrates and pushes that issue's fix. The default limit from `metadata.bot_same_issue_limit` is three completed fixes; missing or invalid values mean three.

When the completed-attempt count is below the limit, record BOUNCED, create one fix bead for the round, and wake the architect last. Counts 0, 1 and 2 therefore authorize the first, second and third fix beads. The architect dispatches a fresh implementer through its queue; never instruct an existing implementer directly. After integrating and pushing the capture, the architect increments each addressed issue's completed-attempt count, replies where evidence is needed, resolves its thread with GitHub's `resolveReviewThread` mutation, reads back `isResolved=true`, and records those thread ids at the new head before removing the merge blocker. The next shepherd pass probes every bot again. New issues begin at zero and never inherit another issue's count.

When a materially unchanged issue remains actionable with a completed-attempt count at or above the resolved `metadata.bot_same_issue_limit`, do not create another fix bead. With the default limit of three, this prevents a fourth fix bead. Record ESCALATED on merge and feature with the repeated issue identities, attempts, heads, fix beads and unresolved thread URLs. Set the merge bead to `state=waiting_human`, preserve the PR and all claims/evidence required for resumption, release the merge slot, and notify `Main` through `hub` with only the merge bead id. The durable escalation states the human decision and resume transition. Unrelated implementation and landing queues continue.
