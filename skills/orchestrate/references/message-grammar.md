...
|---|---|---|
| `REPORTED` | Evidence delivered, next role's label added -- the node's exit contract is met. An implementer or a completing architect writes it holding the claim and yields; the exit gate releases the claim in the fenced write that stamps the proven push. Other writers clear the assignee as their contract says. An implementer's report also carries `dod=<hash>:<n>:met|unmet` for the live acceptance hash and every numbered item; an unmet item is accompanied by the worker's explanation. | `architect`, `implementer`, `researcher`, `unlisted` -- any claimant with no per-role contract |
...
| `REVIEW` | One reviewer's verdict, written on the node its review wisp links to: `verdict=approve|changes` at a named head and round. A plan review carries `dimension=plan plan=<hash>` for the live epic decomposition; an override carries `dimension=override override=<hash>` for the node's live acceptance; a feature code review carries `nodes=<id>:<hash>:met|unmet,...` for every integrated node at that head. | `reviewer` |
...
| `NOTE` | A durable observation without a state transition: a discovered bead's id, a bead-local default and its revisit trigger, a researcher's answer on the linked node, the reaper's record of a claim it preserved and why, an implementer's `NOTE no-change: <reason>` when a git task needs no edit, `NOTE no-acceptance: released by gate` when a routed bead lacks criteria, `NOTE override requested: <reason>` to request a review override, or `NOTE landed uncovered: merge=<id> head=<sha>` when landing lacks acceptance coverage. | `*` -- every claiming role; the extension's reaper |
