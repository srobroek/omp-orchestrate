 ...
 *
 * A watcher record plus a `bd list --json` snapshot produces a receipt plan.
 * Nothing here mutates a bead: the caller persists `requiredMetadata` before delivery.
 *
 * Codes: 0 resolved/replay/duplicate/control, 1 invalid input, 2 no orchestrate owner
 * (safe to route once to pr-shepherd), 3 ambiguous, invalid, or ineligible ownership
 * (do not reroute). Ownership is independent of dispatch approval and head eligibility.
 */
...
/** A queue record has ambiguous, invalid, or ineligible orchestrate ownership. Exit 3. */
...
function liveNode(entry: unknown): { node: Bead; metadata: Bead } | null {
 if (!isObject(entry) || (entry.status !== "open" && entry.status !== "in_progress" && entry.status !== "blocked")) {
  return null;
 }
 let labels: Set<string>;
 try {
  labels = labelSet(entry);
 } catch {
  throw new ResolutionError("cannot determine orchestrate ownership from invalid node labels");
 }
 if (
  !labels.has("orc-node") &&
  !(labels.has("pr:merge") && isObject(entry.metadata) && entry.metadata.integration_owner === "orchestrate")
 ) {
  return null;
 }
 if (!isObject(entry.metadata)) throw new ResolutionError("orchestrate node has invalid metadata");
 const metadata = entry.metadata;
 if (!("repo" in metadata) && !("pr" in metadata)) {
  let hasQueueReceipt = false;
  for (const field in metadata) {
   if (/^queue_(dispatch|lifecycle)(_|$)/.test(field)) {
    hasQueueReceipt = true;
    break;
   }
  }
  if (!hasQueueReceipt) return null;
 }
 if (typeof metadata.repo !== "string" || !REPOSITORY_RE.test(metadata.repo)) {
  throw new ResolutionError("orchestrate node has invalid metadata.repo");
 }
 const number = nodePr(metadata);
 if (number === null || number < 1) throw new ResolutionError("orchestrate node has invalid metadata.pr");
 return { node: entry, metadata };
}
...
/** Select one integration owner per PR, including an explicitly linked node-to-merge handoff. */
function ensureUniqueNodeOwnership(
 nodes: unknown[],
 repository?: string,
 number?: number,
): { node: Bead; metadata: Bead }[] {
 const groups = new Map<string, { node: Bead; metadata: Bead }[]>();
 for (const entry of nodes) {
  const live = liveNode(entry);
  if (!live) continue;
  const pr = nodePr(live.metadata);
  if (repository !== undefined && (live.metadata.repo !== repository || pr !== number)) continue;
  const identity = `${live.metadata.repo}#${pr}`;
  const group = groups.get(identity);
  if (group) group.push(live);
  else groups.set(identity, [live]);
 }
 const owners: { node: Bead; metadata: Bead }[] = [];
 for (const [identity, candidates] of groups) {
  ...
  if (candidates.length === 2) {
   const merge = candidates.find(
    (candidate) =>
     labelSet(candidate.node).has("pr:merge") &&
     candidate.metadata.integration_owner === "orchestrate" &&
     candidate.metadata.role === "shepherd",
   );
   const source = candidates.find(
    (candidate) => candidate !== merge && labelSet(candidate.node).has("orc-node"),
   );
   if (
    merge &&
    source &&
    !labelSet(source.node).has("pr:merge") &&
    isNonEmptyString(merge.node.id) &&
    isNonEmptyString(source.node.id) &&
    merge.node.id !== source.node.id &&
    isNonEmptyString(merge.metadata.origin_bead) &&
    (merge.metadata.origin_bead === source.node.id ||
     (isNonEmptyString(source.node.parent) && merge.metadata.origin_bead === source.node.parent))
   ) {
    owners.push(merge);
    continue;
   }
  }
  throw new ResolutionError(`duplicate orchestrate node ownership for ${identity}, found ${candidates.length}`);
 }
 return owners;
}
...
function resolveLifecycle(lifecycle: LifecycleEvent, nodesValue: unknown): LifecycleResult {
 ...
 const candidates = ensureUniqueNodeOwnership(
  nodeList(nodesValue),
  pullRequest.repository as string,
...
export function resolve(record: unknown, nodesValue: unknown): ResolveResult {
 ...
 const candidates = ensureUniqueNodeOwnership(nodeList(nodesValue), repository, number);

 const key = `${repository}#${number}@${headSha}`;
 if (candidates.length === 0) throw new UnmatchedError(`no orchestrate node for ${repository}#${number}`);
 const { node, metadata } = candidates[0];
 if (node.status !== "in_progress") {
  throw new ResolutionError(`orchestrate owner for ${key} is not in progress`);
 }
 if (!labelSet(node).has("state:approved")) {
  throw new ResolutionError(`orchestrate owner for ${key} is not approved`);
 }
 if (metadata.head_sha !== headSha) {
  throw new ResolutionError(`orchestrate owner for ${key} has a mismatched approved head`);
 }
...
export function replayUnacknowledged(nodesValue: unknown): HandoffResult[] {
 const nodes = ensureUniqueNodeOwnership(nodeList(nodesValue));
 ...
 for (const { node, metadata } of nodes) {
  ...
  if (node.status !== "in_progress" || !labelSet(node).has("state:approved")) {
   throw new ResolutionError("queued orchestrate owner is not in progress and approved");
  }
...
const DESCRIPTION = [
 ...
 "Resolves live PR ownership before requiring an in-progress approved dispatch owner at the exact head.",
 "Matches lifecycle records to the PR's owner and reconciles crash-replay receipts.",
 "Codes: 0 resolved/replay/duplicate/control, 1 invalid input, 2 no orchestrate owner",
 "(safe to route once to pr-shepherd resolve-queue-event), 3 ambiguous, invalid, or ineligible ownership (do not reroute).",
