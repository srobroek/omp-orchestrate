...
describe("a verdict does not depend on the machine's timezone", () => {
 ...
 test("the newest review at one head decides when timestamps use GitHub's uniform Z spelling", () => {
...
describe("G4 replay of an earlier round's verdict", () => {
 ...
 test("review round metadata alone does not change an implementer's own completion evidence", async () => {
 ...
 test("unknown and malformed predicates cannot satisfy a completion contract", () => {
  ...
  for (const wished of [
   ...
   // Misspelled recognized forms must fail too.
   "comment.verb in [REPORTED",
   "Comment.verb in [REPORTED]",
   "metadata.head-sha",
  ]) {
   expect(satisfies(wished, evidence)).toBe(false);
  }

  // Recognized predicates still fail when their evidence is absent.
...
/** Inherited role names must fall back to the generic contract, never an empty one. */
describe("G4 a role name cannot buy a contract-free exit", () => {
 ...
 test.each(WITH_FLOOR)("a bead that satisfies nothing is judged under role %s", async name => {
  reads.bead = { id: NODE, status: "in_progress", assignee: "", labels: [`agent:${name}`], metadata: {} };
  reads.comments = { [NODE]: [{ text: "note: finished, I think" }] };
